...
docker: push-base maven-all push-gradle dotnet-all nodejs-all push-python go-all
podman: push-base-podman maven-all-podman push-gradle-podman dotnet-all-podman nodejs-all-podman push-python-podman go-all-podman

maven-all: push-maven-jdk11 push-maven-jdk17 push-maven-jdk21
maven-all-podman: push-maven-jdk17-podman push-maven-jdk21-podman
...
build-dotnet-8-podman:
	docker build dotnet -f dotnet/podman/Dockerfile -t $(REGISTRY)/$(NAMESPACE)/builder-dotnet:$(VERSION)-podman-8 \
--build-arg DOTNET_VERSION=8.0
...
--build-arg NODE_VERSION=20.15.1 --build-arg NEW_COREPACK=true
push-nodejs-20-podman: build-nodejs-podman
